...
data <- dplyr::mutate(data, hausdorff = 0, avg_hausdorff = 0, mahalanobis = 0, euclidean = 0, hausdorff_three = 0, avg_hausdorff_three = 0, mahalanobis_three = 0, euclidean_three = 0, ss_index_pt = 0)
```
...
qd_hist <- function(tidied)
{
  ...
  if (len <= 20) {
    dist_data <- vector(mode = "list", choose(len, 2))
  } else {
    dist_data <- vector(mode = "list", 200)
  }
  
  for (i in 1:length(dist_data)) {
    dist_data[i] <- dist(sample_n(pts, 2))
  }
...
```{r}
qd_ssim <- function(tidied1, tidied2)
{
...
for(i in 1:200)
{
  ...
  data$ss_index_pt[i] <- qd_ssim(tidied1, tidied2)
...
for(i in 1:200)
{
  ...
  data$ss_index_pt[i+200] <- qd_ssim(tidied1, tidied2)
